 ...
 *
 * DISPLAY: No PSRAM on Core1 Basic, so a full 320x240x2 sprite (~154 KB)
 *          would exhaust the heap. Draw directly to M5.Lcd inside
 *          startWrite/endWrite. Only the small 310x16 ticker uses a sprite.
 *
...
void draw2() {
    ...
    M5.Lcd.fillRect(229, 24, 5, 10, TFT_RED);
    M5.Lcd.fillRect(229, 37, 5, 10, TFT_GREEN);
    ...
    M5.Lcd.setTextFont(1);  // 8px — stationNames fit without overflow
    for (int i = 0; i < ns; i++) {
        M5.Lcd.setTextColor(i == chosen ? TFT_GREEN : TFT_DARKGREEN, TFT_BLACK);
        M5.Lcd.drawString(stationNames[i], 10, 30 + (i * 24));
    }
    ...
    M5.Lcd.drawString(String(voltage), 238, 37);

    M5.Lcd.setTextFont(2);
    M5.Lcd.setTextColor(grays[11], gray);
    M5.Lcd.drawString("VOLOS 2026", 6, 200);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        M5.Lcd.drawString(letters[i], 223 + (i * 32), 154);
    }
...
void loop() {
    ...
    // Full UI redraw at most every 800ms — keeps audio task uninterrupted
